 ...
 * @public
 */
function request(options, callback, isAsync) {
    var body;
    if (options.body) {
        body = options.body;
        delete options.body;
        ...
        return new Promise(function(resolve, reject) {
            var req = https.request(options, function(res) {
                ...
                res.on("error", function(err) {
                    resolve({err, res, resData:undefined});
                });

                res.on("end", function() {
                    res.req = options;
                    res.req.body = body;
                    resolve({err: undefined, res, resData: JSON.parse(resData)});
                });
            });

            req.on("error", function(err) {
                resolve({err, res: undefined, resData: undefined});
            });
